    ...
    sections: [
      {
        title: '📦 包体积优化',
        items: [
          '所有 UI 图片从 **1024×1024 PNG** 转为 **256×256 WebP**，图片体积从 ~12MB 降到 ~100KB，降了 **99%**',
          'ECharts 改为**按需注册**（只引入饼图所需组件），打包体积从 1.1MB 降到 500KB',
          'Vite **代码分割**，antd/echarts/three 拆成独立 vendor 块，主应用 JS 从 2.9MB 降到 320KB',
          '`@ant-design/icons` 改为**直接引用**单个图标，构建模块从 1822 减到 1465',
          '移除了未使用的 `jsrepo` 依赖',
        ],
      },
      {
        title: '🔄 所有页面都支持选盘了',
        items: [
          '**极简模式**点击"开始扫描"弹出盘符选择弹窗，默认勾选 C 盘',
          '**空间概览**扫描按钮改为弹窗选盘，结果页新增「重新选择盘符」按钮',
          '**大文件分析**扫描按钮弹出盘符选择，不再硬编码 C 盘',
          '**逐项清理**也支持选盘扫描了',
        ],
      },
      {
      ...
      },
      {
        title: '🐛 修修补补',
        items: [
          '修复了盘符中文名乱码的问题（改用 PowerShell `execFileSync` 直调，绕开 cmd.exe 编码问题）',
          '盘符选择弹窗默认没有勾选 C 盘的问题修好了',
        ],
      },
    ],
  },
